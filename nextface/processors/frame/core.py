...
from nextface.typing import ProcessFrames, QueuePayload
from nextface.execution import encode_execution_providers
...
def multi_process_frames(source_paths : List[str], temp_frame_paths : List[str], process_frames : ProcessFrames) -> None:
	...
	with tqdm(total = len(queue_payloads), desc = wording.get('processing'), unit = 'frame', ascii = ' =', disable =nextface.globals.log_level in ['warn', 'error']) as progress:
		progress.set_postfix(
		{
			'execution_providers': encode_execution_providers(nextface.globals.execution_providers),
			'execution_thread_count': nextface.globals.execution_thread_count,
			'execution_queue_count': nextface.globals.execution_queue_count
		})
		with ThreadPoolExecutor(max_workers = nextface.globals.execution_thread_count) as executor:
			...
			queue_per_future = max(len(queue_payloads) // nextface.globals.execution_thread_count * nextface.globals.execution_queue_count, 1)
