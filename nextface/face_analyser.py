...
from nextface import process_manager
from nextface.common_helper import get_first
from nextface.face_helper import estimate_matrix_by_face_landmark_5, warp_face_by_face_landmark_5, warp_face_by_translation, create_static_anchors, distance_to_face_landmark_5, distance_to_bounding_box, convert_face_landmark_68_to_5, apply_nms, categorize_age, categorize_gender
from nextface.face_store import get_static_faces, set_static_faces
from nextface.execution import apply_execution_provider_options
from nextface.download import conditional_download
from nextface.filesystem import resolve_relative_path, is_file
from nextface.thread_helper import thread_lock, thread_semaphore, conditional_thread_semaphore
from nextface.typing import VisionFrame, Face, FaceSet, FaceAnalyserOrder, FaceAnalyserAge, FaceAnalyserGender, ModelSet, BoundingBox, FaceLandmarkSet, FaceLandmark5, FaceLandmark68, Score, FaceScoreSet, Embedding
from nextface.vision import resize_frame_resolution, unpack_resolution
	...
	with thread_lock():
		...
		if FACE_ANALYSER is None:
			if nextface.globals.face_detector_model in ['many', 'retinaface']:
				face_detectors['retinaface'] = onnxruntime.InferenceSession(MODELS.get('face_detector_retinaface').get('path'), providers = apply_execution_provider_options(nextface.globals.execution_providers))
			if nextface.globals.face_detector_model in ['many', 'scrfd']:
				face_detectors['scrfd'] = onnxruntime.InferenceSession(MODELS.get('face_detector_scrfd').get('path'), providers = apply_execution_provider_options(nextface.globals.execution_providers))
			if nextface.globals.face_detector_model in ['many', 'yoloface']:
				face_detectors['yoloface'] = onnxruntime.InferenceSession(MODELS.get('face_detector_yoloface').get('path'), providers = apply_execution_provider_options(nextface.globals.execution_providers))
			...
			if nextface.globals.face_recognizer_model == 'arcface_blendswap':
				face_recognizer = onnxruntime.InferenceSession(MODELS.get('face_recognizer_arcface_blendswap').get('path'), providers = apply_execution_provider_options(nextface.globals.execution_providers))
			if nextface.globals.face_recognizer_model == 'arcface_inswapper':
				face_recognizer = onnxruntime.InferenceSession(MODELS.get('face_recognizer_arcface_inswapper').get('path'), providers = apply_execution_provider_options(nextface.globals.execution_providers))
