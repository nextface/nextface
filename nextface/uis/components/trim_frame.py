...
from nextface.vision import count_video_frame_total
...
def render() -> None:
	...
	if is_video(nextface.globals.target_path):
		...
		trim_frame_start_slider_args['value'] = nextface.globals.trim_frame_start or 0
		trim_frame_start_slider_args['maximum'] = video_frame_total
		trim_frame_start_slider_args['visible'] = True
		trim_frame_end_slider_args['value'] = nextface.globals.trim_frame_end or video_frame_total
...
def remote_update() -> Tuple[gradio.Slider, gradio.Slider]:
	if is_video(nextface.globals.target_path):
		video_frame_total = count_video_frame_total(nextface.globals.target_path)
		nextface.globals.trim_frame_start = None
		nextface.globals.trim_frame_end = None
...
def update_trim_frame_start(trim_frame_start : int) -> None:
	nextface.globals.trim_frame_start = trim_frame_start if trim_frame_start > 0 else None


def update_trim_frame_end(trim_frame_end : int) -> None:
	video_frame_total = count_video_frame_total(nextface.globals.target_path)
	nextface.globals.trim_frame_end = trim_frame_end if trim_frame_end < video_frame_total else None
