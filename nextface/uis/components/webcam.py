...
from nextface.audio import create_empty_audio_frame
from nextface.content_analyser import analyse_stream
from nextface.filesystem import filter_image_paths
from nextface.typing import VisionFrame, Face, Fps
from nextface.face_analyser import get_average_face
from nextface.processors.frame.core import get_frame_processors_modules, load_frame_processor_module
from nextface.ffmpeg import open_ffmpeg
from nextface.vision import normalize_frame_color, read_static_images, unpack_resolution
from nextface.uis.typing import StreamMode, WebcamMode
from nextface.uis.core import get_ui_component, get_ui_components
...
def start(webcam_mode : WebcamMode, webcam_resolution : str, webcam_fps : Fps) -> Generator[VisionFrame, None, None]:
	nextface.globals.face_selector_mode = 'one'
	nextface.globals.face_analyser_order = 'large-small'
...
def multi_process_capture(source_face : Face, webcam_capture : cv2.VideoCapture, webcam_fps : Fps) -> Generator[VisionFrame, None, None]:
	with tqdm(desc = wording.get('processing'), unit = 'frame', ascii = ' =', disable =nextface.globals.log_level in ['warn', 'error']) as progress:
		with ThreadPoolExecutor(max_workers = nextface.globals.execution_thread_count) as executor:
			futures = []
