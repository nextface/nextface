...
def render() -> None:
	...
	is_target_image = is_image(nextface.globals.target_path)
	is_target_video = is_video(nextface.globals.target_path)
	TARGET_FILE = gradio.File(
		...
		value = nextface.globals.target_path if is_target_image or is_target_video else None
	)
...
def update(file : File) -> Tuple[gradio.Image, gradio.Video]:
	...
	if file and is_image(file.name):
		nextface.globals.target_path = file.name
	...
	nextface.globals.target_path = None
